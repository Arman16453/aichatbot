...
async def create_session(session_data: Optional[SessionCreate] = None):
    ...
    try:
        ...
        try:
            ...
            if database.db is None or not database.is_connected:
            ...
            if not result.inserted_id:
                print("Failed to get inserted_id from MongoDB")
                raise HTTPException(
                    status_code=500,
            ...
            print(f"Database error while creating session: {str(db_error)}")
            raise HTTPException(
                status_code=500,
